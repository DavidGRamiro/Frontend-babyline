  ...
  onSubmit() {
    ...
    if (this.productoForm.invalid) {
      this._messageService.add({
        severity: 'error',
        summary: 'Error',
        detail: 'Existen campos obligatorios sin rellenar',
      });
      return;
    }
      ...
      this._productoService.altaProducto(this.productoForm.value).subscribe({
        next: (data) => {
          ...
          this._messageService.add({
            ...
            detail: 'Producto dado de alta correctamente',
          });
        },
        error: (error) => {
          this._messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Error al dar de alta el producto',
          });
        },
      });
    } else {
      this._productoService
        .editarProducto(this.producto_formulario.id, this.productoForm.value)
        .subscribe({
          next: (data) => {
            // Mensaje de creación y emitimos evento para cerrar el modal
            this.eventRes.emit(false);
